...
library ChestMetadata {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "experience", name: "ChestMetadata", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x7462657870657269656e63650000000043686573744d65746164617461000000);
   ...
   * @notice Get the full data.
   */
  function _get(bytes32 entityId) internal view returns (ChestMetadataData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = entityId;
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes32 entityId, ChestMetadataData memory _table) internal {
    bytes memory _staticData;
    EncodedLengths _encodedLengths = encodeLengths(_table.name, _table.description);
    bytes memory _dynamicData = encodeDynamic(_table.name, _table.description);
