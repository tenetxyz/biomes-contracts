...
library Exchanges {
   ...
   */
  function encodeLengths(bytes32[] memory exchangeIds) internal pure returns (EncodedLengths _encodedLengths) {
    // Lengths are effectively checked during copy by 2**40 bytes exceeding gas limits
    unchecked {
      _encodedLengths = EncodedLengthsLib.pack(exchangeIds.length * 32);
    }
  }
   ...
   */
  function encodeDynamic(bytes32[] memory exchangeIds) internal pure returns (bytes memory) {
    return abi.encodePacked(EncodeArray.encode((exchangeIds)));
  }
   ...
   */
  function encode(bytes32[] memory exchangeIds) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData;
    EncodedLengths _encodedLengths = encodeLengths(exchangeIds);
    bytes memory _dynamicData = encodeDynamic(exchangeIds);
