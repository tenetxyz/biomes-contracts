...
struct ForceFieldApprovalsData {
  address[] players;
  address[] nfts;
}

library ForceFieldApprovals {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "experience", name: "ForceFieldApprov", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x7462657870657269656e636500000000466f7263654669656c64417070726f76);
   ...
   * @notice Get the full data.
   */
  function _get(bytes32 entityId) internal view returns (ForceFieldApprovalsData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = entityId;
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes32 entityId, ForceFieldApprovalsData memory _table) internal {
    bytes memory _staticData;
    EncodedLengths _encodedLengths = encodeLengths(_table.players, _table.nfts);
    bytes memory _dynamicData = encodeDynamic(_table.players, _table.nfts);
  ...
  ) internal pure returns (ForceFieldApprovalsData memory _table) {
