...
import { BlockExperienceEntityData, BlockExperienceEntityDataWithGateApprovals, BlockExperienceEntityDataWithExchanges, BlockExperienceEntityDataWithPipeControls } from "./../../Types.sol";
...
interface IReadSystem {
  ...
  function experience__getBlockEntityDataWithPipeControls(
    bytes32 entityId
  ) external view returns (BlockExperienceEntityDataWithPipeControls memory);
  ...
  function experience__getBlocksEntityDataWithPipeControls(
    bytes32[] memory entityIds
  ) external view returns (BlockExperienceEntityDataWithPipeControls[] memory);
}
