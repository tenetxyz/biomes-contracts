...
import { CoordMap, VoxelCoord, roundTowardsZero } from "@latticexyz/utils";
...
export function getHeight({ x, z }: VoxelCoord, biome: [number, number, number, number], perlin: Perlin): number {
  ...
  // const maxBiome = biome.findIndex((x) => x === Math.max(...biome));
  // Create valleys
  // if (maxBiome === Biome.Mountains || maxBiome === Biome.Forest) {
  if (biome[Biome.Mountains] > 0 || biome[Biome.Forest] > 0) {
