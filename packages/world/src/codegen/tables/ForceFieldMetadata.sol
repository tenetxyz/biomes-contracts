...
library ForceFieldMetadata {
   ...
   */
  function encodeStatic(uint128 totalMassInside) internal pure returns (bytes memory) {
    return abi.encodePacked(totalMassInside);
  }
   ...
   */
  function encode(uint128 totalMassInside) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData = encodeStatic(totalMassInside);
