...
library PlayerMetadata {
  ...
  function setLastActionTime(bytes32 entityId, uint256 lastActionTime) internal {
    ...
    StoreSwitch.setStaticField(_tableId, _keyTuple, 1, abi.encodePacked((lastActionTime)), _fieldLayout);
  }
  ...
  function _setLastActionTime(bytes32 entityId, uint256 lastActionTime) internal {
    ...
    StoreCore.setStaticField(_tableId, _keyTuple, 1, abi.encodePacked((lastActionTime)), _fieldLayout);
  }
   ...
   */
  function set(bytes32 entityId, bool isLoggedOff, uint256 lastActionTime) internal {
    bytes memory _staticData = encodeStatic(isLoggedOff, lastActionTime);
   ...
   * @notice Set the full data using individual values.
   */
  function _set(bytes32 entityId, bool isLoggedOff, uint256 lastActionTime) internal {
    bytes memory _staticData = encodeStatic(isLoggedOff, lastActionTime);
  ...
  function set(bytes32 entityId, PlayerMetadataData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.isLoggedOff, _table.lastActionTime);
   ...
   */
  function decodeStatic(bytes memory _blob) internal pure returns (bool isLoggedOff, uint256 lastActionTime) {
    isLoggedOff = (_toBool(uint8(Bytes.getBytes1(_blob, 0))));

    lastActionTime = (uint256(Bytes.getBytes32(_blob, 1)));
  }
  ...
  function decode(
    ...
    (_table.isLoggedOff, _table.lastActionTime) = decodeStatic(_staticData);
  }
   ...
   */
  function encodeStatic(bool isLoggedOff, uint256 lastActionTime) internal pure returns (bytes memory) {
    return abi.encodePacked(isLoggedOff, lastActionTime);
  }
