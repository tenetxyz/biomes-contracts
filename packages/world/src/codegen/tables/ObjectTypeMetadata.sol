...
library ObjectTypeMetadata {
  ...
  // Hex-encoded value schema of (uint16, bool, uint16, uint16, uint32, uint32)
  ...
  function getObjectCategory(uint16 objectTypeId) internal view returns (ObjectCategory objectCategory) {
    ...
    return ObjectCategory(uint16(bytes1(_blob)));
  }
  ...
  function setObjectCategory(uint16 objectTypeId, ObjectCategory objectCategory) internal {
    ...
    StoreSwitch.setStaticField(_tableId, _keyTuple, 0, abi.encodePacked(uint16(objectCategory)), _fieldLayout);
  }
  ...
  function _setObjectCategory(uint16 objectTypeId, ObjectCategory objectCategory) internal {
    ...
    StoreCore.setStaticField(_tableId, _keyTuple, 0, abi.encodePacked(uint16(objectCategory)), _fieldLayout);
  }
  ...
  {
    objectCategory = ObjectCategory(uint16(Bytes.getBytes1(_blob, 0)));

    programmable = (_toBool(uint16(Bytes.getBytes1(_blob, 1))));

    stackable = (uint16(Bytes.getBytes1(_blob, 2)));

    maxInventorySlots = (uint16(Bytes.getBytes1(_blob, 3)));
