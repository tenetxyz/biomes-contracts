...
library Position {
   ...
   */
  function get(EntityId entityId) internal view returns (PositionData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = EntityId.unwrap(entityId);
   ...
   * @notice Get the full data.
   */
  function _get(EntityId entityId) internal view returns (PositionData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = EntityId.unwrap(entityId);
   ...
   */
  function set(EntityId entityId, PositionData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(EntityId entityId, PositionData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.x, _table.y, _table.z);
