...
import { EntityData, EntityDataWithBaseEntity, EntityDataWithOrientation, InventoryObject } from "./../../Types.sol";
...
interface IReadSystem {
  ...
  function getEntityDataWithBaseEntity(bytes32 entityId) external view returns (EntityDataWithBaseEntity memory);
  ...
  function getEntityDataWithOrientation(bytes32 entityId) external view returns (EntityDataWithOrientation memory);

  function getEntityDataWithOrientationAtCoord(
    VoxelCoord memory coord
  ) external view returns (EntityDataWithOrientation memory);
