...
function buildObjectAtCoord(uint8 objectTypeId, VoxelCoord memory coord) returns (bytes32) {
...
function buildObjectAtCoordWithOrientation(
  uint8 objectTypeId,
  ...
  bytes32 playerEntityId;
  {
    ...
    (playerEntityId, playerCoord) = requireValidPlayer(WorldContextConsumerLib._msgSender());
  ...
  if (numRelativePositions > 0) {
    ...
    for (uint256 i = 0; i < numRelativePositions; i++) {
      VoxelCoord memory relativeCoord = getRelativeCoord(
        coord,
        rotation,
        VoxelCoord(schemaData.relativePositionsX[i], schemaData.relativePositionsY[i], schemaData.relativePositionsZ[i])
      );
