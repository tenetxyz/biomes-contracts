...
import { ObjectType, PlayerObjectID, ChestObjectID, SmartChestObjectID } from "../ObjectType.sol";
...
function addToInventoryCount(
  ...
  uint16 numInitialObjects = InventoryCount._get(ownerEntityId, objectType);
  ...
  require(numFinalSlotsUsed <= ObjectTypeMetadata._getMaxInventorySlots(ownerObjectType), "Inventory is full");
  InventorySlots._set(ownerEntityId, numFinalSlotsUsed);
  InventoryCount._set(ownerEntityId, objectType, numFinalObjects);

  if (numInitialObjects == 0) {
    InventoryObjects._push(ownerEntityId, objectType.unwrap());
  }
}

function removeFromInventoryCount(EntityId ownerEntityId, ObjectType objectType, uint16 numObjectsToRemove) {
  uint16 numInitialObjects = InventoryCount._get(ownerEntityId, objectType);
  ...
  if (numFinalObjects == 0) {
    InventoryCount._deleteRecord(ownerEntityId, objectType);
    removeObjectTypeFromInventoryObjects(ownerEntityId, objectType);
  } else {
    InventoryCount._set(ownerEntityId, objectType, numFinalObjects);
  }
}

function removeAnyFromInventoryCount(EntityId playerEntityId, ObjectType objectType, uint16 numObjectsToRemove) {
  uint16 remaining = numObjectsToRemove;
  ObjectType[] memory objectTypes = objectType.getObjectTypes();
  for (uint256 i = 0; i < objectTypes.length; i++) {
    uint16 owned = InventoryCount._get(playerEntityId, objectTypes[i]);
    uint16 spend = owned > remaining ? remaining : owned;
    if (spend > 0) {
      removeFromInventoryCount(playerEntityId, objectTypes[i], spend);
      remaining -= spend;
    }
  }
...
function useEquipped(EntityId entityId) returns (uint128 massUsed, ObjectType inventoryObjectType) {
  EntityId inventoryEntityId = Equipped._get(entityId);
  if (inventoryEntityId.exists()) {
    inventoryObjectType = ObjectType._get(inventoryEntityId);
    require(inventoryObjectType.isTool(), "Inventory item is not a tool");
    ...
    if (massLeft <= massUsed) {
      ...
      removeFromInventoryCount(entityId, inventoryObjectType, 1);
...
function removeObjectTypeFromInventoryObjects(EntityId ownerEntityId, ObjectType removeObjectType) {
  uint16[] memory currentObjectTypes = InventoryObjects._get(ownerEntityId);
  uint16[] memory newObjectTypes = new uint16[](currentObjectTypes.length - 1);
  uint256 j = 0;
  for (uint256 i = 0; i < currentObjectTypes.length; i++) {
    if (currentObjectTypes[i] != removeObjectType.unwrap()) {
      newObjectTypes[j] = currentObjectTypes[i];
      j++;
    }
  }
  if (newObjectTypes.length == 0) {
    ...
    InventoryObjects._set(ownerEntityId, newObjectTypes);
  }
}
...
function transferAllInventoryEntities(
  ...
  uint16[] memory fromObjectTypes = InventoryObjects._get(fromEntityId);
  for (uint256 i = 0; i < fromObjectTypes.length; i++) {
    uint16 objectTypeCount = InventoryCount._get(fromEntityId, ObjectType.wrap(fromObjectTypes[i]));
    addToInventoryCount(toEntityId, toObjectType, ObjectType.wrap(fromObjectTypes[i]), objectTypeCount);
    removeFromInventoryCount(fromEntityId, ObjectType.wrap(fromObjectTypes[i]), objectTypeCount);
...
function transferInventoryNonEntity(
  ...
  require(transferObjectType.isBlock() || transferObjectType.isItem(), "Object type is not a block or item");
  require(numObjectsToTransfer > 0, "Amount must be greater than 0");
  removeFromInventoryCount(srcEntityId, transferObjectType, numObjectsToTransfer);
  addToInventoryCount(dstEntityId, dstObjectType, transferObjectType, numObjectsToTransfer);
}
...
function transferInventoryEntity(
  ...
  ObjectType inventoryObjectType = ObjectType._get(inventoryEntityId);
  removeFromInventoryCount(srcEntityId, inventoryObjectType, 1);
  addToInventoryCount(dstEntityId, dstObjectType, inventoryObjectType, 1);
