...
contract InventorySystem is System {
  function drop(EntityId caller, SlotAmount[] memory slotAmounts, Vec3 coord) public {
    require(slotAmounts.length > 0, "Must drop at least one object");
    caller.activate();
    ...
    InventoryUtils.transfer(caller, entityId, slotAmounts);
  ...
  function pickup(EntityId caller, SlotAmount[] memory slotAmounts, Vec3 coord) public {
    caller.activate();
    caller.requireConnected(coord);
    ...
    InventoryUtils.transfer(entityId, caller, slotAmounts);
  }
}
