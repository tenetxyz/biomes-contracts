...
contract ForceFieldSystem is System {
  ...
  function contractForceField(
    ...
    require(validateSpanningTree(boundaryShards, parents), "Invalid connectivity path");
  ...
  function computeBoundaryShards(
    ...
    // Retrieve coordinates using xyz() to get int256 values
   ...
   */
  function validateSpanningTree(Vec3[] memory boundaryShards, uint256[] memory parents) internal pure returns (bool) {
    ...
    require(parents.length == n, "Invalid parents length");
    require(parents[0] == 0, "Root must be self-referential");
    ...
    for (uint256 i = 1; i < n; i++) {
      ...
      // Parent must be in valid range and already visited
      require(parent < n, "Invalid parent index");
      require(visited[parent], "Disconnected node");

      // Verify adjacency
      require(boundaryShards[parent].inVonNeumannNeighborhood(boundaryShards[i]), "Non-adjacent shards");
