...
contract DropSystem is System {
  ...
  function drop(EntityId callerEntityId, ObjectTypeId dropObjectTypeId, uint16 numToDrop, Vec3 coord) public {
    ...
    notify(
      callerEntityId, DropNotifData({ dropCoord: coord, dropObjectTypeId: dropObjectTypeId, dropAmount: numToDrop })
    );
  }
  ...
  function dropTool(EntityId callerEntityId, EntityId toolEntityId, Vec3 coord) public {
    ...
    ObjectTypeId toolObjectTypeId =
      transferInventoryEntity(callerEntityId, entityId, ObjectType._get(entityId), toolEntityId);
  ...
  function dropTools(EntityId callerEntityId, EntityId[] memory toolEntityIds, Vec3 coord) public {
    ...
    for (uint256 i = 0; i < toolEntityIds.length; i++) {
      ObjectTypeId currentToolObjectTypeId =
        transferInventoryEntity(callerEntityId, entityId, ObjectTypes.Air, toolEntityIds[i]);
      if (i > 0) {
