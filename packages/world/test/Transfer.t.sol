...
contract TransferTest is MudTest, GasReporter {
  ...
  function testTransferChestTooFar() public {
    ...
    VoxelCoord memory chestCoord = VoxelCoord(
      spawnCoord.x + MAX_PLAYER_INFLUENCE_HALF_WIDTH + 1,
      spawnCoord.y,
      spawnCoord.z
    );
    bytes32 chestEntityId = testGetUniqueEntity();
    ...
    vm.expectRevert("TransferSystem: destination too far");
    world.transfer(playerEntityId, chestEntityId, inputObjectTypeId1, 1);
  ...
  function testTransferInvalidArgs() public {
    ...
    world.transfer(playerEntityId, chestEntityId, inputObjectTypeId1, 0);
