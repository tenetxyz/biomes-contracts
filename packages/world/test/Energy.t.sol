...
import { CHUNK_SIZE, MAX_PLAYER_INFLUENCE_HALF_WIDTH, WORLD_BORDER_LOW_X, PLAYER_ENERGY_DRAIN_INTERVAL, MACHINE_ENERGY_DRAIN_INTERVAL } from "../src/Constants.sol";
...
contract EnergyTest is BiomesTest {
  ...
  function testPlayerLosesEnergyWhenIdle() public {
    ...
    // pass some time
    vm.warp(block.timestamp + PLAYER_ENERGY_DRAIN_INTERVAL + 1);
    world.activatePlayer(alice);

    uint128 energyAfter = Energy.getEnergy(aliceEntityId);
  ...
  function testMachineLosesEnergyWhenIdle() public {
    ...
    EntityId forceFieldEntityId = setObjectAtCoord(forceFieldCoord, ForceFieldObjectID);
    Energy.setEnergy(forceFieldEntityId, 1000);
    ...
    // pass some time
    vm.warp(block.timestamp + MACHINE_ENERGY_DRAIN_INTERVAL + 1);
    world.activate(forceFieldEntityId);

    uint128 energyAfter = Energy.getEnergy(forceFieldEntityId);
    uint128 energyGainedInPool = LocalEnergyPool.get(shardCoord.x, 0, shardCoord.z) - localEnergyPoolBefore;
    assertTrue(energyGainedInPool > 0, "Local energy pool did not gain energy");
    assertEq(
